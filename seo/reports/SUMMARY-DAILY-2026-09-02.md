# PIXELON SEO — DAILY SUMMARY · 2026-09-02

> Toplam/anonim metrikler. Sorgu, sıralama ve fırsat detayı **bilinçli olarak** dışarıda
> bırakılmıştır — repo public. Detay: `seo/private/reports/DAILY-2026-09-02.md` (yalnızca yerel).

```text
Organic clicks:            27
Organic impressions:       2919
CTR:                       0.92%
Average position:          26.8
Measured window:           28 gün
Indexed URLs:              104
Technical errors (P0/P1):  0/0
Orphan pages:              0
Broken internal links:     0
High-priority opportunities: 41
Approval queue:            0
Checks run:                0
Checks OK:                 0
Checks needing attention:  0
Checks blocked (no data):  0
Plan progress:             38/75
```

## Data sources

```text
Google Search Console: YES
SEMrush:               YES
Live SERP:             YES
Google Search Central: YES
```

## Published today

No production content published.
