/**
 * Platform kartlarının ızgara genişlikleri.
 *
 ...
 */
export function platformSpans(items: readonly { featured?: boolean | undefined }[]): number[] {
  ...
  base.forEach((span, index) => {
    if (width + span > PLATFORM_COLUMNS && row.length > 0) {
      rows.push(row);
      row = [];
      width = 0;
    }
  ...
  for (const indices of rows) {
    ...
    let left = PLATFORM_COLUMNS - used;
